...
class LocalPlanner:
    ...
    def __init__(self,
        ...
        self._local_path_publisher = rospy.Publisher(
            "/carla/%s/local_path" % self.role_name,
            Path,
            queue_size=10
        )
    ...
    def run_step(self):
        ...
        ## ------------------- NOMINAL -------------------
        ...
        elif (self.behavior == Behavior.NOMINAL or self.behavior == Behavior.RED_LIGHT_STOP) and len(self.local_waypoints) < 2:
            ...
            for d_path, d_dot_path, _, s_path, s_dot_path, _ in paths:
                ...
                if free_path:
                    poses = []
                    self.local_waypoints = []
                    for d, d_dot, s, s_dot in zip(d_path[2:], d_dot_path[2:], s_path[2:], s_dot_path[2:]):
                        x, y, yaw, v = self.frenet2world(s, s_dot, d, d_dot)
                        self.local_waypoints.append((x, y, yaw, v))
                    ...
                    msg = Path()
                    msg.poses = poses
                    self._local_path_publisher.publish(msg)
                    break
        
        ## ------------------- RED LIGHT STOP -------------------

        # elif self.behavior == Behavior.RED_LIGHT_STOP and len(self.local_waypoints) < 2:
        #     s, s_dot, d, d_dot = self.world2frenet(self.ego_vehicle_pose, self.ego_vehicle_speed)
        ...
        ## ------------------- LEFT LANE CHANGE -------------------
        ...
        elif self.behavior == Behavior.LANE_CHANGE_TO_LEFT and len(self.local_waypoints) < 2:
            s, s_dot, d, d_dot = self.world2frenet(self.ego_vehicle_pose, self.ego_vehicle_speed)
            ...
            ## generate the possible paths in frenet frame
            param = (c_d, c_s, t_s, self.ds_dot, self.n_ds_dot)
            paths = self.planner.LEFT_LANE_CHANGE(*param)
        ...
        x, y, _, v = self.local_waypoints[0]
        control_msg = self.controller.run_step(v, #20,
        ...
        sampling_radius = 20 * 1 / 3.6          # search radius for next waypoints in seconds
        min_distance = sampling_radius * 0.2    # 20% of the sampling radius

        for i, (x, y, _, _) in enumerate(self.local_waypoints):
            if math.sqrt((x-self.ego_vehicle_pose[0])**2 + (y-self.ego_vehicle_pose[1])**2) < min_distance:
                max_index = i
        if max_index >= 0:
            self.local_waypoints = self.local_waypoints[max_index+1:]
