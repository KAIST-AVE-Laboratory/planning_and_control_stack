...
class FrenetPlanner:
    ...
    def plan_distance_keeping(self,
        ...
        """
        tspace = np.arange(-self.n_dt * self.dt, (self.n_dt + 1) * self.dt, self.dt)
        dspace = np.arange(-self.n_dd * self.dd, (self.n_dd + 1) * self.dd, self.dd)
        sspace = np.arange(-n_ds * ds, (n_ds + 1) * ds, ds)
