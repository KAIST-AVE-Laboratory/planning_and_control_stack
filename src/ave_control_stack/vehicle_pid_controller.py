...
NOTE: This module is copied from carla_ad_agent ros package. However, we modify the
datatype needed to generate controller.
"""
...
class PIDLateralController(object):  # pylint: disable=too-few-public-methods
    ...
    def run_step(self, current_pose, waypoint):
        ...
        end_x = x + math.cos(yaw)
        end_y = y + math.sin(yaw)

        v_vec = np.array([end_x - x, end_y - y, 0.0])
        w_vec = np.array([wpx - x, wpy - y, 0.0])
